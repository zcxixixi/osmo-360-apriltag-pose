...
const root=path.dirname(fileURLToPath(import.meta.url)),timeline=JSON.parse(fs.readFileSync(argv.timeline,'utf8')),fps=Number(argv.fps||timeline.fps),duration=Math.min(Number(argv.duration||timeline.duration_s),timeline.duration_s),frames=Math.round(duration*fps);
...
const browser=await puppeteer.launch({executablePath:'/usr/bin/google-chrome',headless:true,args:['--no-sandbox','--disable-dev-shm-usage','--use-gl=angle','--use-angle=swiftshader','--enable-webgl']});const page=await browser.newPage();await page.setViewport({width:1920,height:1080,deviceScaleFactor:1});await page.goto(`http://127.0.0.1:${port}/?frames=${frames}`,{waitUntil:'networkidle0'});await page.waitForFunction('window.rendererReady===true');
...
for(let index=0;index<frames;index++){await page.evaluate(i=>window.renderFrame(i),index);const png=await page.screenshot({type:'png'});if(!ffmpeg.stdin.write(png))await new Promise(resolve=>ffmpeg.stdin.once('drain',resolve));if(index%60===0)process.stderr.write(`render ${index}/${frames}\n`)}ffmpeg.stdin.end();const code=await new Promise(resolve=>ffmpeg.on('close',resolve));await browser.close();server.close();if(code!==0)throw new Error(`ffmpeg exited ${code}`);console.log(JSON.stringify({output:path.resolve(argv.output),frames,fps,duration_s:duration},null,2));
