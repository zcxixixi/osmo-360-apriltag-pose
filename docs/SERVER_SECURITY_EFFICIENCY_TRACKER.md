...
| 项目 | 当前证据 |
|---|---|
| 流水线 | 服务器正式基线为 `dual-x5-four-mp4-cpu-v14`；当前待发布候选为 `dual-x5-four-mp4-cpu-v16`，继续保持 29.97 Hz、锁定 FFmpeg 9.0.1、AprilTag 原始灰度亮度面和 AprilTag 主导/IMU 有界辅助。v16 增加混合镜头 PnP 失败后的单镜头恢复、遮挡后强几何恢复以及不改变时间戳的零相位轨迹滤波；尚未覆盖服务器正式 `processed/` 产品 |
...
| QUAL-007 | 高 | IN_PROGRESS | 黑夹爪/黄点数据 `155535` 的双色类型锁定正确；进一步逐帧审计确认 6–16 s 的右侧低可信主要来自 Tag 在 back/forward 边缘交替进入，以及 12 s 左侧遮挡后弱平面解在 12.8 s 进入错误分支。v16 候选增加直接 Tag 优先、混合镜头失败回退到有效单镜头、强视觉不被 IMU 残差单独否决、遮挡恢复约束和零相位平滑；预览中 12.8 s 闭合异常已修复，但尚未由用户确认并正式发布。 | 不降低质量门、不把长间隙冒充实测。等待用户确认 `instaumi_20260901_155535_trajectory_zero_phase_filtered_preview_v15.mp4`；确认后以 v16 在服务器无缓存重跑、检查 29.97 Hz/SELF_CALIBRATED_PASS/逐帧位姿并原子发布正式 CSV。 |
...
### 2026-09-03 / Cycle 029

- 用户要求先看 `155535` 的现有轨迹再决定算法修改；本轮没有改动定位、IMU、质量门、CSV 或渲染算法，也没有重新求解/发布失败轨迹。直接使用 v14 失败任务保留的 `tracking/joint_trajectory.csv` 与逐帧质量状态生成诊断视频，长于 0.25 s 的插值仍显示为 `UNTRUSTED`，没有伪装成实测。
- 诊断视频已直接保存在原数据集 `processed/instaumi_20260901_155535_trajectory_diagnostic_world_flu_front_above_v14.mp4`：1920×1080、30 FPS、1,452 帧、48.400 s、22,697,322 B，SHA-256 `79b715c24d1855f94d2648546381e4afbb56decc1abac51874a795aad3386787`。audit 为固定 `flu-front-above --reframe-world-flu`、`WORLD FLU / CAMERA FLU`，世界原点和轴向符合当前产品定义；渲染 wall 64.71 s、平均 CPU 416%、峰值 RSS 1,464,876 KiB、无 swap。封面和 7.1 s 抽帧已人工核对，四路视频、两块 AprilGrid、双轨迹和 XYZ/RPY 面板均可见。
- “可信率”是确定性质量门通过帧比例，不是概率置信度。`155535` 的面板自标定 48/19 inliers、位置 P95 0.0512 m、姿态 P95 3.128°均过门；左侧 1,367/1,448=94.41% 直接观测通过，右侧 1,076/1,446=74.41%。右侧 370 个被拒观测由角残差 `>2°` 163 帧、时间门 207 帧组成；时间门中视觉/IMU 弱观测姿态差 `>15°` 157 帧、拒绝后的强几何重定位不足 45 帧、镜头切换运动越界 5 帧。联合时间线 1,452 帧全部有数值位姿，但只有 1,010 帧双侧实测或不超过 0.25 s 的可信桥接，故联合可信率 69.56%；442 帧保留数值但明确不可信。
- 等用户观看后再决定修复。候选方向必须分别验证 H5 时间校准/IMU一致性、右侧高角残差与重定位锁，不以降低 85% 总门、2°角残差、15°弱视觉/IMU门或 0.25 s 桥接上限作为默认方案。

### 2026-09-03 / Cycle 030

- 按用户对 `155535` 6–16 s 和 12.8 s 跳变的逐帧反馈实现 v16 候选。解码、AprilTag 灰度输入、30 Hz 产品时间线和 AprilTag 主导的融合顺序不变；直接解码 Tag 数量足够时不再被光流角点稀释，混合 back/forward PnP 角残差失败时分别拟合物理镜头并保留有效单镜头解。强 `>=5 Tag` 视觉几何不因 IMU 姿态残差单独拒绝，稀疏镜头交接仍维持严格一致性门。
- 遮挡恢复只允许在可信双端点内部使用强光流恢复，长缺口仍要求直接 Tag 重定位；弱平面解用时间先验选择连续分支。联合轨迹末端增加 9 帧、二阶 centered Savitzky-Golay 零相位滤波，位置和四元数均不移动帧或时间戳；仅对返回相邻可信锚点、最大偏离超过 `max(50 mm, 3×锚点位移)` 的长不可信闭合异常执行锚点内修复，原始左右 tracking CSV 继续保留为审计源。
- `155535` 候选求解保持 1,452 帧联合时间线全部有数值位姿，联合可信率 89.6694%，追踪状态 `SELF_CALIBRATED_PASS`。检测到左侧 12.8128–14.4811 s 闭合异常，锚点位移 29.09 mm、原始最大偏离 206.11 mm，73 帧受有界修复；6–16 s 左侧单步 P95/最大从未滤波的 18.43/71.86 mm 降为 7.13/15.43 mm，右侧为 9.51/25.02 mm。滤波未修改任一时间戳，窗口约 0.3003 s，因此不会引入相位延迟或改变动作发生时间。
- 审阅片保存在原数据集 `processed/instaumi_20260901_155535_trajectory_zero_phase_filtered_preview_v15.mp4`：1920×1080、30 FPS、1,452 帧、48.415 s，SHA-256 `53345c0c3ce12003f6331b4bbe1697a4d017baa7c745aaad7358f6004632cba4`；固定世界 FLU 正面斜上机位，XYZ/RPY 面板可见。该文件名保留生成时的 v15 标签；代码修订已递增到 v16，确保以后不会误复用同名 v15 tracking cache。
- 本地完整测试为 `347 passed, 7 skipped`，聚焦测试 57 passed/1 skipped，`./umi verify` PASS。当前只生成预览，未覆盖服务器正式 `processed/trajectory.csv`，也未宣称完成 v16 无缓存发布；下一步由用户观看确认后再执行正式闭环。
