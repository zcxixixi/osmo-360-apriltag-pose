...
| SEC-002 | 高 | OPEN | 服务器流水线以高权限 `ps` 用户运行。实测 `/var/run/docker.sock` 为 `root:docker 0660`，`ps` 属于 `docker` 组且无需 sudo 即可在关键 `gitlab` 容器中启动 `uid=0(root)` 进程；该容器把 `/srv/gitlab/config`、日志和数据以 RW 挂载到容器，其中 TLS 私钥在容器内虽为 `root:root 0600`，Docker 权限仍可绕过文件权限。`ps` 还属于 `sudo`、`lxd`、`k3s-admin`，因此被攻破的流水线进程可影响整机和 GitLab，不只是当前数据集。本轮未读取任何私钥/业务秘密。 | 建立专用无登录流水线服务账户：不得加入 `sudo/docker/lxd/k3s-admin`，不得访问 Docker socket；代码和输入只读，只通过专用组/ACL 写精确 cache/final/任务槽，并由受管 unit 施加资源、文件系统和网络限制。先做权限/回滚矩阵和 dry-run，再经用户授权迁移；不能直接从当前交互账户删组或中断其他业务。 |
