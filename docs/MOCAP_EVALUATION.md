...
`tools/evaluate_mocap_ground_truth.py` compares a
`tools/osmo_360_offline.py` pose CSV with an OptiTrack/Vicon rigid-body trajectory. It reports:
...
```bash
uv run python -m tools.evaluate_mocap_ground_truth \
