...
function itemMeta(i){const source=i.metrics.source||{},left=source.left||'',right=source.right||'',m=left.match(/VID_(\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})(\d{2})/);if(!m)return{folder:'来源待确认',title:'旧审核数据',detail:'缺少原始文件名',sort:i.pair_id};return{folder:`${m[1]}-${m[2]}-${m[3]}`,title:`${m[4]}:${m[5]}:${m[6]}`,detail:`左 ${left} · 右 ${right}`,sort:m.slice(1).join('')+i.pair_id}}
function renderList(){renderFilters();const shown=items.filter(i=>filter==='all'||status(i)===filter),groups={};shown.forEach(i=>{const m=itemMeta(i);(groups[m.folder]??=[]).push([i,m])});list.innerHTML=Object.keys(groups).sort().map(folder=>`<section><div class="folder-title">📁 ${esc(folder)}　${groups[folder].length} 条</div>${groups[folder].sort((a,b)=>a[1].sort.localeCompare(b[1].sort)).map(([i,m])=>`<div class="item ${current===i.pair_id?'active':''}" onclick="openItem('${i.pair_id}')"><div class="row"><h3>🎬 ${esc(m.title)}</h3><span class="badge ${status(i)}">${labels[status(i)]||labels.pending}</span></div><small>${Math.round(i.metrics.duration_s||i.metrics.rgb_samples||0)} 秒</small><small class="item-source" title="${esc(m.detail)}">${esc(m.detail)}${i.collector?` · 采集 ${esc(i.collector)}`:''}</small></div>`).join('')}</section>`).join('');summary.textContent=`${items.filter(i=>status(i)!=='pending').length} / ${items.length}`}
...
async function openItem(id){current=id;currentData=await api(`/api/items/${id}`);const saved=await api(`/api/items/${id}/alignment`);manualOffset=Number(saved.right_time_offset_s||0);videoFps=Number(currentData.metrics.video_fps||30);alignmentOffset.value=manualOffset.toFixed(3);renderAlignment();empty.hidden=true;detail.hidden=false;const meta=itemMeta(currentData);title.textContent=`${meta.folder} / ${meta.title}`;const aligned=!!currentData.metrics.aligned_video_ready;left.hidden=right.hidden=aligned;leftVideo.hidden=rightVideo.hidden=!aligned;alignmentDetails.hidden=!aligned;if(aligned){leftVideo.src=`/api/items/${id}/video?role=left`;rightVideo.src=`/api/items/${id}/video?role=right`;leftVideo.onplay=()=>{syncRight(true);rightVideo.play().catch(()=>{})};leftVideo.onpause=()=>rightVideo.pause();leftVideo.ontimeupdate=()=>{syncRight();updateTime(leftVideo.currentTime)}}slider.max=Math.max(0,Number(currentData.metrics.duration_s||currentData.metrics.rgb_samples||0));slider.step=(1/videoFps).toFixed(6);slider.value=0;reviewer.value=localStorage.reviewer||currentData.reviewer||'';collector.value=currentData.collector||localStorage.collector||'';updateTime(0);const s=status(currentData);reviewState.textContent=labels[s]||labels.pending;trimPanel.hidden=s!=='approved';yesButton.disabled=!currentData.metrics.review_ready;await loadKeyframes();renderList()}
...
async function saveUsability(usable){message.textContent='';const name=reviewer.value.trim();if(!name){message.textContent='请填写审核人';return}localStorage.reviewer=name;localStorage.collector=collector.value.trim();try{await api(`/api/items/${current}/review`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({decision:usable?'approved':'rejected',reasons:usable?[]:['other'],notes:'',reviewer:name,collector:collector.value.trim()})});items=await api('/api/items');if(usable)await openItem(current);else nextItem()}catch(e){message.textContent=e.message}}
async function addKeyframe(label){const name=reviewer.value.trim(),markTime=commonTime(),markName=label==='useful_start'?'开始':'结束';if(!name){keyframeMessage.textContent='请填写审核人';return}keyframeMessage.textContent=`正在保存${markName} ${markTime.toFixed(3)} 秒…`;try{await api(`/api/items/${current}/keyframes`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({time_sec:markTime,label,reviewer:name})});keyframeMessage.textContent=`已保存${markName} ${markTime.toFixed(3)} 秒`;await loadKeyframes()}catch(e){keyframeMessage.textContent=e.message}}
            ...
            try:
                ...
                if segment_action:
                    ...
                    result = store.add_review(
                        ...
                        notes=str(payload.get("notes", "")),
                        reviewer=str(payload.get("reviewer", "")),
                        collector=str(payload.get("collector", "")),
                    )
