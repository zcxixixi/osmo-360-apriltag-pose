...
from run_physical_two_tag_camera_experiment import (
    compose, inverse, local_corners, pose_from_tag, stats,
)
from run_two_tag_synthetic_experiment import verify_freeze
...
def run_cached(cache_path: Path, tag_map: Path, initial: Path, output: Path, min_tags: int, stride: int, regularize: bool) -> None:
    command=[str(ROOT/".venv/bin/python"),str(ROOT/"raw_fisheye_world_pose_cached.py"),"--observation-cache",str(cache_path),"--tag-map",str(tag_map),"--initial-pose",str(initial),"--sample-stride",str(stride),"--min-tags",str(min_tags),"--max-angular-rmse-deg","1.5","--prior-policy","initial-first","--output-dir",str(output)]
